...
def train(model, train_dataloader, device, blur):
...
filename = 'trials/Colored/' + trialNumber + '/trainingLogColoredNonLinearBlurModel2.csv'
results = [["Epoch", "Training Acc", "Val Acc", "Test Acc", "Time", "Best Val Acc"]]
for epoch in range(numEpochs):
    model.train()
    train_dataloader.dataset.change_img_lst(epoch)
    blur = get_blur(epoch, epochs_to_final=50, sigma_nonlin=True, base = 2)

    start_time = time.time()
    model, num_correct_training = train(model, train_dataloader, device, blur)
    ...
    if (num_correct_validation/len(val_dataset) > best_accuracy):
        torch.save(model.state_dict(), 'trials/Colored/' + trialNumber + '/ColoredNonLinearBlurModel2.pt')

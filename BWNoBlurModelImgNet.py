...
model = resnet50()
loaded_state_dict = torch.load('/trials/BW/' + trialNumber + '/BWNoBlurModel.pt')
'''
...
filename = 'trials/BWImgNet/' + trialNumber + '/trainingLogBWNoBlurModel.csv'
...
for epoch in range(numEpochs):
    ...
    if (num_correct_validation/len(val_dataset) > best_accuracy):
        torch.save(model.state_dict(), 'trials/BWImgNet/' + trialNumber + '/BWNoBlurModel.pt')

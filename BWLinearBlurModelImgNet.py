...
model = resnet50()
loaded_state_dict = torch.load('trials/BW/' + trialNumber + '/BWLinearBlurModel.pt')
'''
...
filename = 'trials/BWImgNet/' + trialNumber + '/trainingLogBWLinearBlurModel.csv'
...
for epoch in range(numEpochs):
    ...
    if (num_correct_validation/len(val_dataset) > best_accuracy):
        torch.save(model.state_dict(), 'trials/BWImgNet/' + trialNumber + '/BWLinearBlurModel.pt')

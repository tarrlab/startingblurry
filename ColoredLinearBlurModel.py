...
filename = 'trials/Colored/' + trialNumber + '/trainingLogColoredLinearBlurModel.csv'
results = [["Epoch", "Training Acc", "Val Acc", "Test Acc", "Time", "Best Val Acc"]]
for epoch in range(numEpochs):
    model.train()
    train_dataloader.dataset.change_img_lst(epoch)
    blur = get_blur(epoch, epochs_to_final=50)
    ...
    if (num_correct_validation/len(val_dataset) > best_accuracy):
        torch.save(model.state_dict(), 'trials/Colored/' + trialNumber + '/ColoredLinearBlurModel.pt')
